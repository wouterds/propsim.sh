...
describe("toCandles, the bar still printing", () => {
  it("should refuse the live quote row Yahoo stamps at the quote time", () => {
    // given two closed bars, the bar still printing, and Yahoo's quote row
    // sitting off the grid at the moment it was quoted
    const now = JAN_05 + 3 * MINUTE + 47;
    const result = chart(
      [
        solid(JAN_05, 100),
        solid(JAN_05 + MINUTE, 101),
        solid(JAN_05 + 2 * MINUTE, 102),
        { time: now, open: 103, high: 103, low: 103, close: 103, volume: 0 },
      ],
      now,
    );

    // when the caller wants the forming bar
    const candles = toCandles(result, MINUTELY, true);

    // then the off-grid quote is not a bar, and the one still printing is
    expect(candles.at(-1)?.time).toBe((JAN_05 + 2 * MINUTE) * 1000);
    expect(candles.some((candle) => candle.time === now * 1000)).toBe(false);
  });

  it("should leave the bar still printing out unless it is asked for", () => {
    // given the same tape
    const now = JAN_05 + 2 * MINUTE + 30;
    const result = chart(
      [solid(JAN_05, 100), solid(JAN_05 + MINUTE, 101), solid(JAN_05 + 2 * MINUTE, 102)],
      now,
    );

    // then the matcher sees only what closed
    expect(toCandles(result, MINUTELY).at(-1)?.time).toBe((JAN_05 + MINUTE) * 1000);
    expect(toCandles(result, MINUTELY, true).at(-1)?.time).toBe((JAN_05 + 2 * MINUTE) * 1000);
  });
});
